...
const QUIZ = {
 ...
 "questions": [
  ...
  },
  {
   ...
   "topic": "1.3 · ordered",
   "type": "single",
   "text": "Вставляем пачку из 50 новых документов вызовом <code>insert_many</code> без дополнительных параметров. У двадцатого документа <code>_id</code> совпадает с ключом, который уже есть в коллекции. Сколько документов из пачки окажется в коллекции?",
   "options": [
  ...
  },
  {
   ...
   "topic": "1.3 · загрузка",
   "type": "single",
   "text": "Скрипт читает файл выгрузки и вызывает <code>insert_many(docs)</code>. У документов в файле нет поля <code>_id</code>. Скрипт по ошибке запустили дважды. Что получится?",
   "options": [
  ...
  },
  {
   ...
   "topic": "1.4 · find_one",
   "type": "single",
   "text": "Смартфонов в коллекции четыре, самый дешёвый из них — Xiaomi за 18 990. Запрос <code>find_one({\"category\": \"смартфоны\"})</code> вернул именно его. Какой вывод верен?",
   "options": [
    "find_one отдаёт самый дешёвый документ из подходящих",
    "find_one отдаёт документ, вставленный в коллекцию первым",
    "Какой из подходящих вернётся, без сортировки не определено",
    "Совпадение случайно: find_one сортирует документы по _id"
   ],
   "key": {
    "python": [
     "2df8eab6a56f1735"
  ...
  },
  {
   ...
   "options": [
    ...
    "На странице 25 мест, а занято из них только пять",
  ...
  },
  {
   ...
   "type": "single",
   "text": "У одного товара цену по ошибке записали строкой <code>\"790\"</code>, у остальных 20 — числа от 690 до 114 990. Где он окажется в выдаче <code>.sort({ price: 1 })</code>?",
   "options": [
    "Первым: 790 меньше почти всех остальных цен",
  ...
  },
  {
   ...
   "text": "Отметьте <b>все</b> верные утверждения.",
   "options": [
    "После delete_many с пустым фильтром индексы коллекции сохраняются",
    "После drop() коллекции нет в списке list_collection_names",
    "Сервер отклоняет delete_many с пустым фильтром как опасный",
